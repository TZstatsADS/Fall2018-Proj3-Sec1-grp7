...
setwd("/Users/hongru/Documents/GitHub/Fall2018-Proj3-Sec1-grp7/Github/doc/")
...
```{r model_setup}
model_values = seq(0.6, 0.8, by = 0.05)
model_labels = paste("randomForest with samplesize =", model_values)
```
...
```{r train_label}
train_label_path <- "/Users/hongru/Documents/GitHub/Fall2018-Proj3-Sec1-grp7/data/train_set/label.csv"
...
`trainRF.R` and `testRF.R` should be wrappers for all your model training steps and your prediction steps. 
+ `train.R`
...
```{r loadlib}
source("../lib/RandomForest/trainRF.R")
source("../lib/RandomForest/testRF.R")
```
...
source("../lib/RandomForest/CVRF.R")

if(run.cv){
  err_cv <- array(dim=c(length(model_values), 2))
  for(k in 1:length(model_values)){
    cat("k=", k, "\n")
    err_cv[k,] <- cv_RF(feat_train, label_train, model_values[k], K)
  }
...
par <- list(samplesize=model_best)
```

* Train the model with the entire training set using the selected model (model parameter) via cross-validation.
```{r final_train}
tm_train=NA
tm_train <- system.time(fit_train <- trainRF(feat_train, label_train, par))
